            ...
            <div class="content" v-loading="loading">
                <div class="total">
                    <el-row>
                        <el-col :span="5">
                            <div>
                                <div>{{pay_data.pay_total}}</div>支付总金额
                            </div>
                        </el-col>
                        <el-col :span="5">
                            <div>
                                <div>{{pay_data.no_total}}</div>未支付总金额
                            </div>
                        </el-col>
                        <el-col :span="4">
                            <div>
                                <div>{{pay_data.back_total}}</div>退款总额
                            </div>
                        </el-col>
                        <el-col :span="5">
                            <div>
                                <div>{{pay_data.channel_before}}</div>分成前渠道收入
                            </div>
                        </el-col>
                        <el-col :span="5">
                            <div>
                                <div>{{pay_data.channel_after}}</div>分成后渠道收入
                            </div>
                        </el-col>
                    </el-row>
                </div>
                ...
                <div class="data-table">
                    ...
                    >
                        ...
                        <el-table-column label="渠道昵称" prop="ditch_name"></el-table-column>
                        <el-table-column label="头像" prop="head_img">
                            <!-- 图片显示 -->
                            <template slot-scope="scope">
                                <img :src="scope.row.head_img" width="40px" class="head_pic">
                            </template>
                        </el-table-column>
                        ...
                        <el-table-column label="订单状态" prop="order_status">
                            <template slot-scope="scope">
                                ...
                                <span v-if="scope.row.order_status==2">已退款</span>
                            </template>
                        </el-table-column>
                        <el-table-column label="支付金额" prop="pay_count"></el-table-column>
...
export default {
    ...
    mounted() {
        ...
        this.getMoneyData();
        //获取订单数据
        this.getOrderData(this.pagesize,this.current_page,this.status,this.date_range,0);
    },
    ...
    methods: {
        ...
        getMoneyData(){
            ...
            this.$axios.post(this.$src+'/admin/orderdata/order_count',parmas).then((result)=>{
        ...
        getOrderData(pagesize,current_page,status,date_range,download){
            ...
            //请求数据
            if(!download){
                this.$axios.post(this.$src+'/admin/orderdata/order_list',parmas).then((result)=>{
                    if(result.data.code=='200'){
            ...
            //下载数据
                ...
                window.location.href=this.$src+'/admin/orderdata/order_excel?'+parmas.toString()
            }
        },

        //下载数据
        download() {
            this.getOrderData(this.pagesize,this.current_page,this.status,this.date_range,1);

        },

        //查询数据
        goSearch() {
            this.getOrderData(this.pagesize,this.current_page,this.status,this.date_range);    
        },
 
        //每页显示几条数据
        handleSizeChange(size) {
            this.pagesize = size;  
            this.getOrderData(size,this.current_page,this.status,this.date_range); 
        },

        //点击第几页
        handleCurrentChange(current_page){
            this.current_page = current_page; 
            this.getOrderData(this.pagesize,current_page,this.status,this.date_range);  
        }
