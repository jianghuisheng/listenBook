                        ...
                        >
                            <el-table-column label="封面图">
                                <template slot-scope="scope">
                                    <img :src="scope.row.cover" height="60px" class="head_pic">
                                </template>
                            </el-table-column>
                            ...
                            <el-table-column label="对应商品" prop="product_name"></el-table-column>
                            <el-table-column label="对应皮肤">
                                <!-- 字体变红色 -->
                                <template slot-scope="scope">
                                    <span style="color:red" v-if="scope.row.skin">{{scope.row.skin}}</span>
                                </template>
                            </el-table-column>
                            <el-table-column label="推荐理由" prop="reason"></el-table-column>

                            <el-table-column label="操作">
                                <template slot-scope="scope">
                                    <el-button type="primary" @click="seeArticle(scope.row.href)">
                                        查看文章
                                    </el-button>
                                </template>
                            </el-table-column>
                    ...
                    <el-tab-pane label="图片素材" name="2">
                        <div class="ele-row-flex">
                            ...
                            >
                                <div>
                                    <div class="img-show">
                                        <img style="width:100%;height:100%" :src="item.url" al="">
                                    </div>
                                    <p class="img-des">{{item.name+'('+item.width+'*'+item.height+')'}}</p>
                                </div>

                                <!-- 判断svg图是否显示 -->
                                <div :class="item.id==show_id?'svg-show':'svg-hidden'">
                                    <div>
                                        <el-button type="primary" @click="downloadPicture(item.url)">下载图片</el-button>
                                    </div>
                                    <div style="color: rgb(0, 0, 0); position: absolute; top: 4px; right: 6px;">
                                        <i class="anticon anticon-fullscreen" style="color: rgb(255, 255, 255); font-size: 18px;">
                                            <svg
                                ...
                                <el-dialog
                                    ...
                                    <div>
                                        <img :src="click_cover" style="width:100%">
                                    </div>
                                </el-dialog>
                            </div>
                        </div>
...
export default {
    ...
    methods: {
        ...
        getArticleData(pagesize,current_page){
            ...
            this.$axios.post(this.$src+'/admin/popularize/popularize',parmas).then((result)=>{
        ...
        getPictureMaterial(){
            ...
            this.$axios.post(this.$src+'/admin/popularize/picture',parmas).then((result)=>{
                if(result.data.code=='200'){
